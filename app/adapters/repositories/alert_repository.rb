...
module Repositories
  class AlertRepository
    ...
    def save_alert(alert)
      # Validate input
      raise ArgumentError, "Alert cannot be nil" unless alert

      # Set up context for error handling
      context = {
        alert_name: alert.name,
        alert_severity: alert.severity.to_s,
        alert_status: alert.status.to_s
      }
      ...
      handle_database_error("save_alert", context) do
    ...
    def find_alert(id)
      ...
      handle_database_error("find_alert", context) do
        # Find in database
        domain_alert = DomainAlert.find_by(id: id_str)

        # Handle the not found case
        if domain_alert.nil?
          # Return nil for a normal "not found" scenario
          # This is appropriate if it's a normal lookup where the result may or may not exist
          return nil
        end
    ...
    def list_alerts(filters = {})
      ...
      handle_query_error("list_alerts", context) do
