  ...
  describe '.valid?' do
    context 'with valid tokens' do
      ...
      it 'returns true for valid GitHub token' do
        expect(WebhookAuthenticator.valid?("github_secret", "github")).to be true
      end

      it 'returns true for valid Jira token' do
        expect(WebhookAuthenticator.valid?("jira_secret", "jira")).to be true
      end

      it 'returns true for valid token for custom source' do
        expect(WebhookAuthenticator.valid?("default_secret", "custom")).to be true
      end
    end
  ...
  describe '.secret_for' do
    context 'with environment variables set' do
      ...
      it 'returns the correct secret for GitHub' do
        expect(WebhookAuthenticator.secret_for("github")).to eq("env_github_secret")
      end

      it 'returns the correct secret for Jira' do
        expect(WebhookAuthenticator.secret_for("jira")).to eq("env_jira_secret")
      end
    end
    ...
    context 'with Rails credentials set' do
      before do
        allow(ENV).to receive(:[]).and_return(nil)
        allow(Rails.application.credentials).to receive(:dig).with(:github, :webhook_secret).and_return("cred_github_secret")
        allow(Rails.application.credentials).to receive(:dig).with(:jira, :webhook_secret).and_return("cred_jira_secret")
      end
